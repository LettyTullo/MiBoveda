...
## 1. Protocolos de Ventana Corrediza
...
> - **$T_t$**: Tiempo de transmisión ($L_t (Longitud de la trama (bits)) / V_t (tasa de bits) (bits/seg)$).
...
## 2. Protocolos de Acceso Múltiple (ALOHA)

> [!warning] ALOHA Puro (Máximo 18.4%)
> $$S = G \cdot e^{-2G}$$

> [!check] ALOHA Ranurado/Slotted (Máximo 36.8%)
> $$S = G \cdot e^{-G}$$

---

## 3. Otros Protocolos Importantes

> [!example] Ethernet Clásico
> $$U = \frac{1}{1 + \frac{2BLe}{cF}}$$

> [!quote] Protocolos de Reserva y Selección
> - **Mapa de Bits (Carga Alta):** $Ef = \frac{d}{d + 1}$
> - **Cuenta Regresiva Binaria:** $Ef = \frac{d}{d + \log_2 N}$
